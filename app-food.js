...
var wishWXInfo = {
    ...
    secret: '4a8666a73cf11387054bc2ef1fdd7631'
};
...
app.get('/openid', function(req, res){
    ...
    superagent.get('https://api.weixin.qq.com/sns/jscode2session?appid='+wishWXInfo.appid+'&secret='+wishWXInfo.secret+'&js_code='+code+'&grant_type=authorization_code')
    .charset('utf-8')
    .end(function (err, sres) {
        if (err) {
            res.json({code: failCode, msg: err});
...
app.get('/accesstoken', function(req, res){
    ...
    MongoClient.connect(DB_CONN_STR1, function(err, db) {
        ...
        var requestNewToken = function(){
            superagent.get('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='+wishWXInfo.appid+'&secret='+wishWXInfo.secret)
            .charset('utf-8')
            .end(function (err, sres) {
                if (err) {
                    res.json({code: failCode, msg: err});
